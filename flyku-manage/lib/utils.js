...
exports.extract = function (sourceUrl) {
    if (!!config.extractTo && !extracted[sourceUrl] &&  //去重
        sourceUrl.indexOf(path.resolve(cwd, config.template)) === -1) { //存在于template中的文件无需拷贝，template会整体拷贝。
        ...
        console.info("#yellow{[extract file]}" + sourceUrl + "\n");
        extracted[sourceUrl] = true;
    }
};
