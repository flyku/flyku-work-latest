...
var publishOnePage = function (pageUrl, toServer) {
    ...
    fs.writeFileSync(projectRoot + '/' + pageName + '.html', htmlCode);
...
var analyzeHtml = function (htmlpath, projectRoot, pageName, toServer) {
    ...
    puzs = puzs.concat(tagExtension.allPuz());
    incs = incs.concat(tagExtension.allInc());
...
exports.publish = function (pageUrl) {
    ...
    if (typeof pageUrl !== 'undefined') {
        pageName = path.basename(path.basename(pageUrl, '.html'), '.htm');
        if (pageName === 'pages') {
            pageName = '';
        }
    }
    ...
    var extracted = {};
    puzs.forEach(function (puzUrl) {
        if (!extracted[puzUrl]) {
            utils.extract(puzUrl);
            extracted[puzUrl] = true;
        }
    });
    ...
    if (incs.length > 0) {
        var inclist = '';
        incs.forEach(function (incUrl) {
            if (!extracted[incUrl]) {
                inclist += incUrl + '\n';
                extracted[incUrl] = true;
            }
        });
        console.info('#green{included html files:\n' + inclist + '}');
    }
};
